<template>
	<view class="customer-box">
		<uni-nav-bar @clickLeft="goBack" left-icon="back" leftIcon="arrowleft" title="地址管理" statusBar="true"
			fixed="true" color="#ffffff" backgroundColor="#333333"></uni-nav-bar>
			...
			<view v-if="addressList && addressList.length > 0" class="address_content">
				...
				<view class="address_liests" v-for="(item, index) in addressList" :key="index">
                  ...
                  getLableVal(item.label)
								...
								<text class="address_word">{{ item.provinceName }}{{ item.cityName
                  ...
                  item.sex === "0"
					...
					<view class="list_item_bottom">
						<label class="radio" @click.stop="getRadio(index, item)">
							<radio class="item_radio" v-if="testValue" color="#FFC200" :value="item.id"
								:checked="item.isDefault === 1 && isActive === index"
								@click.stop="getRadio(index, item)" />设为默认地址
						</label>
					</view>
				</view>
			</view>
			<!-- 无地址展示 -->
			<empty v-if="isEmpty" boxHeight="100%" textLabel="一个地址都没有哦"></empty>
		...
		methods: {
			...
			addOrEdit(type, item) {
				...
				if (type === "新增") {
					...
					// TODO
					uni.redirectTo({
						url: "/pages/addOrEditAddress/addOrEditAddress?type=" +
							"编辑" +
							"&" +
							"id=" +
							item.id,
